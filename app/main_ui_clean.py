...
# Stage 3: The Final Blueprint Review & Edit
    ...
    machine_prompt = """SYSTEM: You are an expert n8n JSON generator. Create a workflow that triggers on a new Outlook Calendar event. The workflow must: 1. Use the Google Calendar node to create an identical event on the calendar office-calendar@mycompany.com. 2. Use the Google Sheets node to search for any existing events in the sheet at https://docs.google.com/spreadsheets/d/123abc... that overlap with the new events start and end times. 3. Use an If node to check if the search found any overlapping events. 4. If a conflict exists: Use two Email nodes to send a conflict alert email to both the new events organizer and the existing events organizer. 5. If no conflict exists: Use the Google Sheets node to append the new booking details as a new row. Then, use a Wait node to pause the workflow until 30 minutes before the events start time. After the wait, use an Email node to send a reminder to the events organizer. The final output must be a single, valid n8n JSON object."""
